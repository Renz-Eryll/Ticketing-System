...
const AgentNotification = () => {
  ...
  const [currentPage, setCurrentPage] = useState(() => {
    const savedPage = localStorage.getItem("agentCurrentPage");
    return savedPage ? parseInt(savedPage) : 1;
  });
  const itemsPerPage = 5;

  useEffect(() => {
    localStorage.setItem("agentCurrentPage", currentPage);
  }, [currentPage]);
  useEffect(() => {
    return () => {
      localStorage.removeItem("agentCurrentPage");
    };
  }, []);
    ...
    const fetchNotif = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:8000/api/agentnotifications/${user.id}`,
          {
  ...
  useEffect(() => {
    const fetchUnreadCount = async () => {
      try {
        const res = await fetch(
          `http://localhost:8000/api/unread-count/${user.id}`,
          {
            headers: {
  ...
  const markNotificationAsRead = async (notifId) => {
    try {
      const res = await fetch(
        `http://localhost:8000/api/updateNotif/${notifId}`,
        {
          method: "PUT",
          headers: {
  ...
  const markAllAsRead = async () => {
    try {
      const res = await fetch(
        `http://localhost:8000/api/notifications/mark-all-read/${user.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
